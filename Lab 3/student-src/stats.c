...
*/
void compute_stats() {
    double access_time = (double) ((MEMORY_READ_TIME * stats.accesses) + (DISK_PAGE_READ_TIME * stats.page_faults) + (DISK_PAGE_WRITE_TIME * stats.writebacks));
    access_time = (double)(access_time / (double) stats.accesses);
    stats.aat = access_time;
}
